...
{
    class GameDataProvider : IResolvableItem
    {
        ...
        public T GetData<T>(string name) where T : class, IGameDataModel
        {
            ...
            {
                _logger.Log("WARNING! no models of type " + requestedType + " exist", true);
                return null;
            }

            if (!_data[requestedType].ContainsKey(name)) {
                _logger.Log("WARNING! model of type " + requestedType + " with name " + name + " does not exist");
                return null;
            }
